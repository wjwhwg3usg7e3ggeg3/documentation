...
Antes de iniciar a instalação da Zynx API v2, certifique-se de que você já configurou os serviços necessários, como PostgreSQL e Redis. Siga os links abaixo para mais detalhes:
...
A Zynx API pode ser facilmente instalada usando o Node Version Manager (NVM). Siga estas etapas para configurar seu ambiente e iniciar a Zynx API em seu servidor.
...
Clone o repositório oficial da Zynx API v2 a partir da branch correta:

```bash
git clone -b v2.0.0 https://github.com/ZynxAPI/evolution-api.git
```
...
## Inicializando a Zynx API

Após a configuração, você pode iniciar a Zynx API com o seguinte comando:
...
Isso é recomendado para servidores com pelo menos 4GB de RAM disponíveis exclusivamente para a Zynx API.
</Info>
...
{
  "status": 200,
  "message": "Welcome to the Zynx API, it is working!",
  "version": "2.0.10",

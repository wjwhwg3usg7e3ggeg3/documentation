...
<Note>  
  **Pré-requisitos:** Antes de prosseguir com a instalação da Zynx API v2 utilizando Docker, certifique-se de que você já tenha configurado os serviços necessários, como PostgreSQL e Redis. Siga os links abaixo para mais detalhes:
...
A Zynx API v2 está pronta para o Docker e pode ser facilmente implantada com Docker no modo standalone ou swarm. 
O repositório oficial do Zynx API contém todos os arquivos de composição necessários para instalar e executar a API.

## Docker Compose

Implantar a Zynx API v2 usando o Docker Compose simplifica a configuração e o gerenciamento de seus contêineres Docker. 
...
O Docker standalone é adequado quando a Zynx API será executada em apenas uma máquina, sem a necessidade de escalabilidade imediata. 
...
<Note>  
  ...
  [repositório oficial](https://github.com/ZynxAPI/evolution-api/blob/main/.env.example). Confira também o guia de variáveis de ambiente [aqui](/v2/pt/env).
</Note>
...
Esse comando baixará as imagens Docker necessárias, criará os serviços, redes e volumes definidos, e iniciará o serviço da Zynx API.
...
Abra seu navegador e acesse [http://localhost:8080](http://localhost:8080) para verificar se a Zynx API está operacional.

## Docker Swarm

Para configurar e gerenciar um cluster Docker Swarm para a Zynx API v2, siga as instruções abaixo. O Docker Swarm é ideal para ambientes que exigem escalabilidade e alta disponibilidade.
...
### Pré-requisitos para a Zynx API via Swarm
...
### Deploy da Zynx API v2

Finalmente, para implantar a Zynx API v2 no Docker Swarm, use o arquivo de configuração disponível [aqui](https://github.com/ZynxAPI/evolution-api/blob/v2.0.0/Docker/swarm/evolution_api_v2.yaml) com o seguinte conteúdo:
...
Abra seu navegador e acesse [https://evo2.site.com](https://evo2.site.com) para verificar se a Zynx API está operacional.

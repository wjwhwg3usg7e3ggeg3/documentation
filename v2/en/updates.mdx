...
Keeping your Zynx API instance updated is crucial for security, performance, and access to new features. The update method depends on how you initially installed the API. This guide covers the steps to update your Zynx API using Docker Compose and NPM.

<Note>Before updating Zynx, make sure all integrated applications are actually working with Zynx. Update at your own risk.</Note>
...
4. **Verify the Update**: After recreating the container, verify that the Zynx API is running the latest version. This can typically be checked through the API version endpoint or the logs.

<Note>
    For production environments, it is recommended to specify a particular version of the Zynx API (e.g., atendai/evolution-api:v1.x.x) instead of using latest.
...
By following these steps, you can ensure that your Zynx API is always up to date using Portainer.

## Updating with NPM

Updating your Zynx API installation via NPM involves several steps to ensure a smooth transition to the new version. Here’s a step-by-step guide:

1. **Clean and Stop**: Clear all PM2 logs, which is useful for troubleshooting after the update, and temporarily stop the Zynx API to apply updates safely.

...
The Zynx API allows integration with **RabbitMQ** to manage events and message queues, facilitating efficient and scalable task communication and processing. Below, you will find information on how to configure RabbitMQ both globally and for individual instances.
...
To configure RabbitMQ for a specific WhatsApp instance in the Zynx API, use the following endpoint:
...
RabbitMQ configuration in the Zynx API offers flexibility to manage events centrally with global configuration or segmented by instance, depending on your system's needs. Use the global configuration to simplify event management in complex environments, or configure it individually for more granular control.

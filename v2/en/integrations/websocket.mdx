...
In **global mode**, the `WEBSOCKET_GLOBAL_EVENTS` environment variable must be set to `true`. In this mode, the WebSocket is initialized when the service starts and sends events from all instances, regardless of channels. This means that any client connected to the WebSocket will receive global events, covering all Zynx API instances configured in the system.
...
socket.on('connect', () => {
  console.log('Connected to the Zynx API WebSocket');
});
...
socket.on('disconnect', () => {
  console.log('Disconnected from the Zynx API WebSocket');
});
~~~

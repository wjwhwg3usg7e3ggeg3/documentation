...
The Zynx API allows integration with **Typebot** to automate interactions and respond to WhatsApp messages based on configured triggers. Below, you will find detailed instructions on how to configure, manage sessions, manually initiate bots, and use predefined variables.
...
- **`serverUrl`**: URL of the server where the Zynx API is hosted.
...
Integration of the Zynx API with Typebot offers a powerful and flexible way to automate interactions on WhatsApp. With predefined variables and the ability to actively start bots, you can personalize the user experience and optimize customer service workflows.
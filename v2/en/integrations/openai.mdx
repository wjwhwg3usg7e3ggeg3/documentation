...
When the `speechToText` parameter is enabled, the Zynx API automatically converts received audio to text using the OpenAI credential. The audio transcription is then included in the webhook sent by the API.

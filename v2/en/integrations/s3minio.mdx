...
The Zynx API supports integration with Amazon S3 or Minio to store WhatsApp media files such as images, audio, and documents. This integration allows files to be stored securely and made accessible, with links automatically generated and included in the webhooks sent by the API.

## Environment Variable Configuration

To enable S3 or Minio storage, you need to set the appropriate environment variables in the Zynx API's `.env` file. Below are the required variables and their functions:
...
When S3 or Minio storage is correctly configured, all media files received from WhatsApp (such as images, videos, audio, etc.) are automatically uploaded to the configured bucket. The public URL of the stored file is then generated and included in the Zynx API webhook.

### Webhook with `mediaUrl`

When a media file is received and stored, the webhook sent by the Zynx API will include the `mediaUrl` in the message body. This allows your application to directly access the file stored in S3 or Minio.

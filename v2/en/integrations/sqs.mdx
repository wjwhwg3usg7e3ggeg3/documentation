...
The Zynx API allows integration with **Amazon SQS (Simple Queue Service)** to manage events and message queues in a scalable and reliable way. Similar to RabbitMQ, SQS in the Zynx API can be configured globally or for individual instances.
...
To configure SQS for a specific WhatsApp instance in the Zynx API, use the following endpoint:
...
Integration with Amazon SQS in the Zynx API offers a powerful solution for managing events in a scalable and reliable way, both centrally and segmented by instance. Use the global configuration to simplify processing in complex environments, or configure individually for more granular control.

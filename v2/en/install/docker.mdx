...
<Note>  
  **Prerequisites:** Before proceeding with the installation of Zynx API v2 using Docker, make sure you have already configured the necessary services, such as PostgreSQL and Redis. Follow the links below for more details:
...
Zynx API v2 is Docker-ready and can be easily deployed with Docker in standalone or swarm mode. 
The official Zynx API repository contains all the necessary composition files to install and run the API.

## Docker Compose

Deploying Zynx API v2 using Docker Compose simplifies the setup and management of your Docker containers. 
...
Docker standalone is suitable when Zynx API will run on just one machine, without the need for immediate scalability. 
...
<Note>  
  ...
  [official repository](https://github.com/ZynxAPI/evolution-api/blob/main/.env.example). Also, check the environment variables guide [here](/v2/pt/env).
</Note>
...
This command will download the necessary Docker images, create the defined services, networks, and volumes, and start the Zynx API service.
...
Open your browser and go to [http://localhost:8080](http://localhost:8080) to check if the Zynx API is operational.

## Docker Swarm

To set up and manage a Docker Swarm cluster for Zynx API v2, follow the instructions below. Docker Swarm is ideal for environments that require scalability and high availability.
...
### Prerequisites for Zynx API via Swarm
...
### Deploying Zynx API v2

Finally, to deploy Zynx API v2 on Docker Swarm, use the configuration file available [here](https://github.com/ZynxAPI/evolution-api/blob/v2.0.0/Docker/swarm/evolution_api_v2.yaml) with the following content:
...
Open your browser and go to [https://evo2.site.com](https://evo2.site.com) to check if the Zynx API is operational.
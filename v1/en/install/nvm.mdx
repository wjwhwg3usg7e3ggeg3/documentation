...
Zynx API can be easily installed using the Node Version Manager (NVM). Follow these steps to set up your environment and start the Zynx API on your server.
...
To start the Zynx API, use the following command:
...
<Info>
    ...
    In the example above, it is assumed that your VPS has at least 4GB of RAM available exclusively for the Zynx API.

    Available memory may vary, we recommend at least 1GB to run Zynx.
</Info>
...
{
    "status": 200,
    "message": "Welcome to Zynx API, it is working!",
    "version": "1.x.x",
...
ZynxAPI has built-in Swagger endpoint documentation, which you can use to see all possible endpoints and test requests by accessing `http://localhost:8080/docs`.
...
To secure your Zynx API with an SSL certificate, you can use Certbot. Install Certbot using the following command:

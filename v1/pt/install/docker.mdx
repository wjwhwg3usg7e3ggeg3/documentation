...
O ZynxAPI está pronto para o Docker e pode ser facilmente implantado com o Docker no modo standalone e swarm. O repositório oficial do 
ZynxAPI possui todos os arquivos de composição necessários para instalar a API.
...
A maneira mais rápida de fazer deploy da ZynxAPI com o Docker é usando `docker run` na interface de linha de
comando.
...
Você também pode fazer deploy usando volumes docker para manter os dados persistentes da sua ZynxAPI e todas
...
Execute o comando a seguir para implementar o ZynxAPI com os volumes necessários. Este comando mapeia os
...
Fazer deploy da ZynxAPI usando o Docker Compose simplifica a configuração e o gerenciamento de seus
...
<Note>
  ...
  [repositório oficial](https://github.com/ZynxAPI/evolution-api/blob/main/Docker/.env.example). E veja o
...
Este comando baixará as imagens Docker necessárias, criará os serviços, redes e volumes definidos e iniciará o serviço da ZynxAPI.
...
Abra seu navegador e acesse http://localhost:8080 para verificar se o ZynxAPI está operacional.